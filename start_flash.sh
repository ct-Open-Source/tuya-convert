...
# Read operating system identification data from /etc/os-release
if [[ -e /etc/os-release ]]; then
        source /etc/os-release
fi

# Check if Debian 12 (Bookworm) or newer
debian12ornewer="false"
...
setup () {
        ...
        if [ "$screen_minor" -gt 5 ]; then
                # Check if Debian 12 (Bookworm) or newer
                if [[ ${debian12ornewer} == 'true' ]] then
                        # Yes, Debian 12 (Bookworm) or newer
                        screen_with_log="sudo --preserve-env=PATH screen -L -Logfile"
                else
